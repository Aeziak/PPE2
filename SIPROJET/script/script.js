...
function setID() {
	...
	setInterval( function courseOn() { // Boucle Programme
		...
		if ( off === 3 ) { // Pour reset toutes les données lors du début de la seconde course
			...
			dTotal = d + dTotal;
			console.log(dTotal);
			// On voit si la distance totale est supérieur ou non à une valeur et on attribue en fonction une valeur à m1
			if ( dTotal > 100) {
				m1 = 1;
			}
			...
			// On calcule de la distance et du temps
			if ( screenIsMap === false ) { // Si la map n'est pas activée
